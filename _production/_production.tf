...
module "production_datadog" {
  source        = "./modules/datadog"
  shared_secret = "${var.datadog_integration_id}"
}

module "production_memcached" {
  ...
  availability_zones     = "${var.availability_zones}"
  subnet_group_ids       = "${module.production_vpc.private_subnets}"
  security_group_ids     = ["${module.production_vpc.internal_inbound_id}"]
...
module "production_bastion" {
  ...
  iam_instance_profile = "${module.production_iam.instance_profile}"
  availability_zones   = "${var.availability_zones}"

  public_subnet_ids  = "${module.production_vpc.public_subnets}"
  bastion_inbound_id = "${module.production_vpc.bastion_inbound_id}"
...
module "production_backup" {
  ...
  availability_zones = "${var.availability_zones}"
  system_name        = "${var.system_name}"
  instance_profile   = "${module.production_iam.instance_profile}"
  instance_role_arn  = "${module.production_iam.instance_role_arn}"
  security_group_ids = ["${module.production_vpc.internal_inbound_id}"]
  subnet_group_ids   = "${module.production_vpc.private_subnets}"

  ansible_playbooks = "${module.production_codedeploy.ansible_playbooks}"
}

module "production_datomic" {
  ...
  availability_zones                = "${var.availability_zones}"
  system_name                       = "${var.system_name}"
  peer_role                         = "${module.production_iam.instance_role_name}"
  security_group_ids                = ["${module.production_vpc.internal_inbound_id}"]
  subnet_group_ids                  = "${module.production_vpc.private_subnets}"
  ...
  memcached_uri                     = "${module.production_memcached.memcached_uri}"
  datadog_api_key                   = "${var.datadog_api_key}"
}

module "production_c2" {
  source           = "./modules/web"
  environment      = "${var.environment}"
  ...
  instance_profile   = "${module.production_iam.instance_profile}"
  instance_role_arn  = "${module.production_iam.instance_role_arn}"
  security_group_ids = ["${module.production_vpc.web_inbound_id}"]
  subnet_group_ids   = "${module.production_vpc.public_subnets}"

  ansible_playbooks = "${module.production_codedeploy.ansible_playbooks}"
}

module "production_chat" {
